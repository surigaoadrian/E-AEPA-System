...
const EvaluationStatusChart = () => {
    ...
    const fetchData = async () => {
      try {
        const [thirdMonthResponse, fifthMonthResponse] = await Promise.all([
          fetch(`${apiUrl}evaluation/thirdMonthStatus`),
          fetch(`${apiUrl}evaluation/fifthMonthStatus`),
        ]);
        ...
        const totalThirdMonthEvaluations =
          thirdMonthData.completed + thirdMonthData.notCompleted;
        const totalFifthMonthEvaluations =
          fifthMonthData.completed + fifthMonthData.notCompleted;

        const calculatePercentages = (completed, total) => {
          const completedPercentage =
            total > 0 ? ((completed / total) * 100).toFixed(2) : 0;
          const notCompletedPercentage =
            total > 0 ? (((total - completed) / total) * 100).toFixed(2) : 0;
          return {
        ...
        const thirdMonthPercentages = calculatePercentages(
          thirdMonthData.completed,
          totalThirdMonthEvaluations
        );
        const fifthMonthPercentages = calculatePercentages(
          fifthMonthData.completed,
          totalFifthMonthEvaluations
        );
  ...
  return (
    <ResponsiveBar
      ...
      role="application"
      barAriaLabel={(e) =>
        `${e.id}: ${e.formattedValue}% in evaluation: ${e.indexValue}`
      }
    />
  );
};
