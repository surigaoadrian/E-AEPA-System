...
import AnnualSecondSemComments from "../modals/AnnualSecondSemComments";
...
const AnnualSecondSemEval = ({ headId, userId, filter, selectedYear, selectedSemester  }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      //setLoading(true); 
      // SELF VALUES
      const fetchSelfValuesAnnualSecond = async () => {
        try {
      ...
      // SELF JOB
      const fetchSelfJobAnnualSecond = async () => {
        try {
          const response = await axios.get(
            `${apiUrl}results/getAverages`,
            {
      ...
      //PEER VALUES
      const fetchPeerAnnualSecond = async () => {
        try {
      ...
      //HEAD JOB
      const fetchHeadJobAnnualSecond = async () => {
        try {
          const response = await axios.get(
              `${apiUrl}results/getJobRespAverageByEmpId`,
              {
                  params: {
                      empId: userId,
                      period: "Annual-2nd", //SUBJECT TO CHANGE "Annual-1st"
                  },
              }
          );
...
// HEAD VALUES
const fetchHeadValuesAnnualSecond = async () => {
  try {
    const response = await axios.get(
    ...
    const data = response.data;
    console.log("HV: ", data);
    if (data.period === "Annual-2nd") { //SUBJECT TO CHANGE "Annual-1st"
      const roundedCultureOfExcellenceAverage = parseFloat(data.cultureOfExcellenceAverage.toFixed(2));
      const roundedIntegrityAverage = parseFloat(data.integrityAverage.toFixed(2));
      const roundedTeamworkAverage = parseFloat(data.teamworkAverage.toFixed(2));
      const roundedUniversalityAverage = parseFloat(data.universalityAverage.toFixed(2));
  ...
  return (
    ...
    <div className="justify-center items-center" >
      ...
      {/* ANNUAL SECOND SEM EVALUATION TAB*/}
