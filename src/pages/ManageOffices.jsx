...
const ManageOffices = () => {
  ...
  const displayedDepartments = departments.slice(startIndex, endIndex); //changed to departments
  ...
  //       try {
  //         const response = await axios.get(`${apiUrl}department/getAllDepts`);
  ...
  const confirmDeleteDepartment = async () => {
    try {
      ...
      const updatedUsers = await axios.get(`${apiUrl}department/getAllDepts`);
    ...
    const handleSubmit = (e) => {
      ...
      const handleUpdateDepartment = async () => {
        try {
          ...
          console.log("Edited department content:", editedDepartment);
          handleRenderFlag();
